...
// ===================== SHADER - Adapted for Navy/Gold Theme =====================
const vertexShader = `
  varying vec2 vUv;
  void main() {
    vUv = uv;
...
// Modified CPPN shader with navy blue and gold color palette
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    // Color transformation: Map to Navy Blue (#0f2c59) and Gold (#f59e0b) palette
    // Navy: rgb(15, 44, 89) = (0.059, 0.173, 0.349)
    // Gold: rgb(245, 158, 11) = (0.961, 0.620, 0.043)

    float intensity = (buf[0].x + buf[0].y + buf[0].z) / 3.0;

    // Create gradient from deep navy to gold highlights
    vec3 navyDeep = vec3(0.02, 0.06, 0.15);    // Very dark navy
    vec3 navyMid = vec3(0.059, 0.173, 0.349);  // Navy-900
    vec3 goldDim = vec3(0.6, 0.4, 0.1);        // Muted gold
    vec3 goldBright = vec3(0.961, 0.620, 0.043); // Gold-500

    vec3 color;
    if (intensity < 0.3) {
      color = mix(navyDeep, navyMid, intensity / 0.3);
    } else if (intensity < 0.6) {
      color = mix(navyMid, goldDim, (intensity - 0.3) / 0.3);
    } else {
      color = mix(goldDim, goldBright, (intensity - 0.6) / 0.4);
    }
...
function ShaderPlane() {
  ...
  useFrame((state) => {
    ...
    // Scale the plane to fill the viewport
    if (meshRef.current) {
      ...
      meshRef.current.scale.set(planeWidth * 1.2, planeHeight * 1.2, 1);
    }
  });
...
export function NeuralNetworkBackground() {
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 z-[1] bg-gradient-to-r from-navy-900/80 via-navy-900/60 to-navy-900/40" />
      <div className="pointer-events-none absolute inset-0 z-[1] bg-gradient-to-t from-navy-900/50 via-transparent to-navy-900/30" />
    </div>
  );
}
