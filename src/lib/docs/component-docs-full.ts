...
export const ADMIN_COMPONENTS: ComponentDoc[] = [
  ...
  {
    name: 'BugMessagesPanel',
    filePath: 'src/components/admin/BugMessagesPanel.tsx',
    description: 'Admin panel for bug report messaging thread. Shows conversation between admin and user, with system messages for status changes. Supports real-time message sending with push notification to user.',
    props: [
      { name: 'bugId', type: 'string', required: true, description: 'Bug report ID' },
      { name: 'bugUserId', type: 'string', required: true, description: 'User who reported the bug' },
      { name: 'bugUserName', type: 'string', required: true, description: 'User display name' },
    ],
    hooks: ['useState', 'useRef', 'useEffect', 'useBugMessages', 'useAddBugMessage'],
    usedBy: ['AdminPage (BugDetailDialog)'],
    uses: ['Card', 'CardContent', 'CardHeader', 'CardTitle', 'Button', 'Textarea', 'Avatar', 'AvatarImage', 'AvatarFallback', 'Badge', 'ScrollArea'],
    stateManagement: 'TanStack Query + local state for message input',
  },
  {
    name: 'PushNotificationsTab',
    filePath: 'src/components/admin/PushNotificationsTab.tsx',
    description: 'Admin tab for testing push notifications. Allows sending test notifications to specific users and broadcasting system announcements to all users with push enabled.',
    props: [],
    hooks: ['useState', 'useAdminUsers'],
    usedBy: ['AdminPage'],
    uses: ['Card', 'CardContent', 'CardHeader', 'CardTitle', 'CardDescription', 'Button', 'Input', 'Label', 'Textarea', 'Badge', 'Select', 'SelectContent', 'SelectItem', 'SelectTrigger', 'SelectValue'],
    stateManagement: 'Local state + TanStack Query for users',
  },
];
...
export const NOTIFICATION_COMPONENTS: ComponentDoc[] = [
  ...
  },
  {
    ...
    filePath: 'src/components/notification-preferences.tsx',
    description: 'Full notification settings panel. Master push toggle with category toggles (bugs, team, achievements, announcements, reminders). Handles iOS PWA requirements and permission states.',
    props: [
      { name: 'className', type: 'string', required: false, description: 'Additional CSS classes' },
    ],
    hooks: ['useState', 'useEffect', 'useUser', 'usePushNotifications'],
    usedBy: ['ProfilePage', 'SettingsPage'],
    uses: ['Card', 'CardContent', 'CardHeader', 'CardTitle', 'Switch', 'Label'],
    stateManagement: 'localStorage for preferences, usePushNotifications hook',
  },
  {
    name: 'PushNotificationToggle',
    filePath: 'src/components/push-notification-toggle.tsx',
    description: 'Simple push notification toggle with informational tooltip. Shows platform-specific messages (iOS PWA required, blocked, not supported).',
    props: [
      { name: 'className', type: 'string', required: false, description: 'Additional CSS classes' },
      { name: 'showDescription', type: 'boolean', required: false, defaultValue: 'true', description: 'Show descriptive text' },
    ],
    hooks: ['usePushNotifications'],
    usedBy: ['ProfilePage', 'SettingsPage', 'NotificationPreferencesPanel'],
    uses: ['Switch', 'Label', 'Tooltip'],
    stateManagement: 'usePushNotifications hook',
  },
  {
    name: 'PushNotificationButton',
    filePath: 'src/components/push-notification-toggle.tsx',
    description: 'Compact push notification toggle button for header/nav. Icon-only with tooltip.',
    props: [
      { name: 'className', type: 'string', required: false, description: 'Additional CSS classes' },
    ],
    hooks: ['usePushNotifications'],
    usedBy: ['AppLayout header'],
    uses: ['Button', 'Tooltip'],
    stateManagement: 'usePushNotifications hook',
  },
  {
    name: 'PushPermissionPrompt',
    filePath: 'src/components/push-permission-prompt.tsx',
    description: 'Smart contextual permission prompt dialog. Shows at appropriate moments (first habit, bug submitted, etc) with benefits list. Follows 2025 best practice of explaining value before asking.',
    props: [
      { name: 'context', type: 'PromptContext', required: true, description: 'Trigger context (first_habit_complete, bug_submitted, etc)' },
      { name: 'open', type: 'boolean', required: false, description: 'Controlled open state' },
      { name: 'onOpenChange', type: '(open: boolean) => void', required: false, description: 'Open state handler' },
      { name: 'onEnabled', type: '() => void', required: false, description: 'Called when user enables' },
      { name: 'onDismissed', type: '() => void', required: false, description: 'Called when user dismisses' },
    ],
    hooks: ['useState', 'useEffect', 'usePushNotifications'],
    usedBy: ['DashboardPage', 'BugReportingDialog'],
    uses: ['Dialog', 'DialogContent', 'DialogHeader', 'DialogTitle', 'DialogDescription', 'DialogFooter', 'Button'],
    stateManagement: 'localStorage for shown contexts and cooldown, controlled/uncontrolled',
  },
  {
    name: 'SystemAnnouncementBanner',
    filePath: 'src/components/system-announcement-banner.tsx',
    description: 'Dismissible system announcement banner with video support. Gradient backgrounds by type (info, warning, success). Safe area aware for PWA notch.',
    props: [
      { name: 'enabled', type: 'boolean', required: true, description: 'Show/hide banner' },
      ...
      { name: 'videoUrl', type: 'string', required: false, description: 'Optional video guide URL' },
      { name: 'type', type: "'info' | 'warning' | 'success'", required: false, defaultValue: "'info'", description: 'Banner style' },
      { name: 'storageKey', type: 'string', required: false, description: 'localStorage key for dismissal' },
      { name: 'onDismiss', type: '() => void', required: false, description: 'Callback when dismissed' },
      { name: 'isTopBanner', type: 'boolean', required: false, defaultValue: 'true', description: 'Apply safe-area-inset padding' },
    ],
    hooks: ['useState', 'useEffect'],
    usedBy: ['AppLayout'],
    uses: ['Button', 'Dialog', 'DialogContent', 'DialogHeader', 'DialogTitle'],
    stateManagement: 'localStorage for dismissal state',
  },
  {
    name: 'SystemStatusBadge',
    filePath: 'src/components/system-announcement-banner.tsx',
    description: 'Compact status badge shown when full announcement banner is dismissed. Clickable to show announcement again.',
    props: [
      { name: 'enabled', type: 'boolean', required: true, description: 'Show/hide badge' },
      { name: 'title', type: 'string', required: true, description: 'Badge title' },
      { name: 'type', type: "'info' | 'warning' | 'success'", required: false, defaultValue: "'info'", description: 'Badge color' },
