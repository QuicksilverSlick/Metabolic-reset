  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="text-center max-w-2xl mx-auto">
        <h1 className="text-3xl font-display font-bold text-navy-900 mb-4">Find Your Team</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredCaptains.length > 0 ? (
          filteredCaptains.map((captain) => (
            <Card key={captain.id} className="border-slate-200 hover:border-orange-200 hover:shadow-md transition-all">
              <CardHeader className="pb-4">
                <div className="w-12 h-12 bg-navy-100 rounded-full flex items-center justify-center mb-4 text-navy-700 font-bold text-xl">
                  {captain.name.charAt(0)}
                </div>
                <CardTitle className="text-lg">{captain.name}</CardTitle>
                <CardDescription>Code: <span className="font-mono font-bold text-orange-600">{captain.referralCode}</span></CardDescription>
              </CardHeader>
              <CardContent>
                <Button 
